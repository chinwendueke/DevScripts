...
then
	...
	#Remove docker
	sudo apt-get install -y git vim net-tools sysstat zip wget finger

elif [[ "$DISTRO" =~ "$COS" ]]
then
        echo " OS: $DISTRO Detected"
	sudo yum install -y git vim net-tools sysstat zip wget finger
elif [[ "$DISTRO" =~ "$ALP" ]]
then
        echo " OS: $DISTRO Detected"
	sudo apk add -y -y git vim net-tools sysstat zip wget finger
else
